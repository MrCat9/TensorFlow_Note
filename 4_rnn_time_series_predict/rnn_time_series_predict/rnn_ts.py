...
class SeriesPredictor:
    ...
    def model(self):
        ...
        outputs, states = tf.nn.dynamic_rnn(cell, self.x, dtype=tf.float32)  # outputs shape=(?, 5, 100)=(batch, seq_size, hidden)
        num_examples = tf.shape(self.x)[0]
        W_repeated = tf.tile(tf.expand_dims(self.W_out, 0), [num_examples, 1, 1])
        out = tf.matmul(outputs, W_repeated) + self.b_out  # shape=(?, 5, 1)
        out = tf.squeeze(out)  # shape=(?, 5)
        return out
    ...
    def train(self, train_x, train_y, test_x, test_y):
        with tf.Session() as sess:
            ...
            max_patience = 3  # 出现连续 max_patience 次训练后cost没有减小，则停止训练（一次训练迭代100次）
            patience = max_patience
            min_test_err = float('inf')
            step = 0
            while patience > 0:
                _, train_err = sess.run([self.train_op, self.cost], feed_dict={self.x: train_x, self.y: train_y})
                if step % 100 == 0:  # 每 100 次检查一次
                    test_err = sess.run(self.cost, feed_dict={self.x: test_x, self.y: test_y})
                    print('step: {}\t\ttrain err: {}\t\ttest err: {}'.format(step, train_err, test_err))
                    if test_err < min_test_err:  # 训练后cost减小
...
if __name__ == '__main__':
    ...
    for i in range(len(train_data) - seq_size):
        train_x.append(np.expand_dims(train_data[i:i + seq_size], axis=1).tolist())  # shape=(batch, seq_size, input_dim)
